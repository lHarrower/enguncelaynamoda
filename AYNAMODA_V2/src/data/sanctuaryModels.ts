...
export const getCategoryCompatibility = (category1: ClothingCategory, category2: ClothingCategory): number => {
...
export const calculateOutfitConfidence = (items: ClothingItem[]): number => {
  ...
  const colorHarmonyBonus = items.length > 1 ? 
    items.reduce((harmony, item, index) => {
      if (index === 0) return harmony;
      return harmony + getColorCompatibility(items[0].colors, item.colors);
    }, 0) / (items.length - 1) : 0;
  
  // Bonus for category compatibility
  const categoryBonus = items.length > 1 ?
    items.reduce((compatibility, item, index) => {
      if (index === 0) return compatibility;
      return compatibility + getCategoryCompatibility(items[0].category, item.category);
    }, 0) / (items.length - 1) : 0;
...
export const getSeasonalRecommendations = (season: 'spring' | 'summer' | 'fall' | 'winter'): ClothingCategory[] => {
