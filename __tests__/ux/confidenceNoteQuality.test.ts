...
describe('Confidence Note Quality - User Experience Tests', () => {
  ...
  describe('Confidence Note Content Quality', () => {
    it('should generate personalized confidence notes based on user history', async () => {
        ...
        previousFeedback: [
          {
            ...
            confidenceRating: 5,
            emotionalResponse: { primary: 'confident', intensity: 9 },
      ...
      const encouragingWords = ['confident', 'amazing', 'perfect', 'great', 'beautiful', 'stunning', 'ready'];
    ...
    it('should reference previous positive experiences when available', async () => {
      const mockOutfit = {
        id: 'outfit-456',
        items: [
          { id: 'item-3', category: 'dress', colors: ['red'], tags: ['formal'] }
        ],
        confidenceScore: 4.8
      };
      ...
      const referencesHistory = confidenceNote.toLowerCase().includes('last time') ||
    ...
    it('should encourage rediscovery of neglected items', async () => {
      ...
      const rediscoveryWords = ['forgotten', 'neglected', 'rediscover', 'time to', 'bring back', 'dust off'];
  ...
  describe('Confidence Note Accessibility', () => {
    it('should generate screen-reader friendly confidence notes', async () => {
      const mockOutfit = {
        ...
        items: [
          { id: 'item-6', category: 'shirt', colors: ['navy'], brand: 'J.Crew' }
        ],
        confidenceScore: 4.3
      };
  ...
  describe('Cultural Sensitivity and Inclusivity', () => {
    ...
    it('should work well for users with different style preferences', async () => {
      ...
      for (const preference of stylePreferences) {
        ...
        const acknowledgesStyle = confidenceNote.toLowerCase().includes(preference.style) ||
