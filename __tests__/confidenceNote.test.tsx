...
describe('ConfidenceNote', () => {
  ...
  describe('Rendering', () => {
    it('should render confidence note text', () => {
      const { getByText } = render(
        <ConfidenceNote note={mockNote} confidenceScore={4.2} />
      );
  ...
  describe('Confidence Levels', () => {
    it('should display exceptional confidence for high scores', () => {
      const { getByText } = render(
        <ConfidenceNote note={mockNote} confidenceScore={4.8} />
      );
    ...
    it('should display moderate confidence for average scores', () => {
      const { getByText } = render(
        <ConfidenceNote note={mockNote} confidenceScore={3.2} />
      );
    ...
    it('should display building confidence for lower scores', () => {
      const { getByText } = render(
        <ConfidenceNote note={mockNote} confidenceScore={2.5} />
      );
  ...
  describe('Confidence Bar', () => {
    it('should display confidence bar with correct fill percentage', () => {
      const { UNSAFE_getByType } = render(
        <ConfidenceNote note={mockNote} confidenceScore={4.0} />
      );
    ...
    it('should cap confidence bar at 100%', () => {
      const { UNSAFE_getByType } = render(
        <ConfidenceNote note={mockNote} confidenceScore={6.0} />
      );
  ...
  describe('Glow Effects', () => {
    ...
    it('should not show glow effect for lower confidence levels', () => {
      const { UNSAFE_getByType } = render(
        <ConfidenceNote note={mockNote} confidenceScore={3.5} />
      );
  ...
  describe('Text Handling', () => {
    it('should handle long confidence notes', () => {
      const longNote = 'This is a very long confidence note that should wrap properly and display beautifully even when it contains multiple sentences and lots of encouraging words to boost the user\'s confidence throughout their day.';
      
      const { getByText } = render(
        <ConfidenceNote note={longNote} confidenceScore={4.2} />
      );
    ...
    it('should handle short confidence notes', () => {
      ...
      const { getByText } = render(
        <ConfidenceNote note={shortNote} confidenceScore={4.2} />
      );
    ...
    it('should handle empty confidence notes gracefully', () => {
      const { UNSAFE_getByType } = render(
        <ConfidenceNote note="" confidenceScore={4.2} />
      );
    ...
    it('should handle special characters and emojis', () => {
      ...
      const { getByText } = render(
        <ConfidenceNote note={emojiNote} confidenceScore={4.2} />
      );
