...
export class AynaAIService {
  ...
  private static filterByMood(wardrobe: ClothingItem[], mood: MoodTag): ClothingItem[] {
    ...
    return wardrobe.filter((item) => item.colors.some((color) => preferredColors.includes(color)));
  }
  ...
  private static generateBasicOutfit(items: ClothingItem[], mood: MoodTag): Outfit {
    ...
    for (const category of priorities) {
      const categoryItems = items.filter(
        (item) => item.category === category && !outfit.includes(item),
      );
    ...
    for (const existingItem of existingOutfit) {
      ...
      const colorScore = Math.max(
        ...item.colors.map((color1) =>
          Math.max(
            ...existingItem.colors.map((color2) => getColorCompatibility([color1], [color2])),
          ),
        ),
      );
  ...
  static generateDailyOutfits(wardrobe: ClothingItem[], count: number = 3): Outfit[] {
    ...
    for (let i = 0; i < count && i < moods.length; i++) {
      ...
      const outfit = this.generateOutfit(wardrobe, mood);
      if (outfit) {
        outfits.push(outfit);
      }
    }
  ...
  static calculateWardrobeStats(wardrobe: ClothingItem[]): WardrobeStats {
    ...
    for (const item of wardrobe) {
      ...
      _totalWearCount += item.wearCount;
    ...
    const [mostUsedCategory] = Object.entries(categoryCounts).sort(([, a], [, b]) => b - a)[0] || [
      'tops',
      0,
    ];
    ...
    return {
      ...
      mostUsedCategory: mostUsedCategory || undefined,
      colorHarmony,
    } as WardrobeStats;
  }
  ...
  static generateInsights(wardrobe: ClothingItem[], stats: WardrobeStats): AynaInsight[] {
    ...
    const forgottenItems = wardrobe.filter(
      (item) => !item.lastWorn || item.lastWorn < thirtyDaysAgo,
    );
