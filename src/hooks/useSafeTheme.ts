...
export function useSafeTheme() {
  ...
  const safeTheme = {
    ...
    colors: {
      ...
      primary:
        typeof theme.colors.primary === 'object' && theme.colors.primary !== null
          ? (theme.colors.primary as Record<string, string>)[500] ||
            (theme.colors.primary as Record<string, string>).main ||
            '#5C8A5C'
          : theme.colors.primary || '#5C8A5C',
      // Ensure primaryIndexed is available for components that need color scales
      primaryIndexed: theme.colors.primaryIndexed ||
        theme.colors.sage || {
          500: '#5C8A5C',
          400: '#7FA87F',
          300: '#A8C8A8',
      ...
      surface:
        typeof theme.colors.surface === 'object'
