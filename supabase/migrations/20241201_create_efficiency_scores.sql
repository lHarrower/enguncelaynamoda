...
-- Migration: 20241201_create_efficiency_scores.sql
...
CREATE INDEX idx_efficiency_scores_user_id ON efficiency_scores(user_id);
CREATE INDEX idx_efficiency_scores_created_at ON efficiency_scores(created_at DESC);
CREATE INDEX idx_efficiency_scores_overall_score ON efficiency_scores(overall_score DESC);
CREATE INDEX idx_efficiency_scores_user_created ON efficiency_scores(user_id, created_at DESC);

-- Efficiency goals indexes
CREATE INDEX idx_efficiency_goals_user_id ON efficiency_goals(user_id);
CREATE INDEX idx_efficiency_goals_type ON efficiency_goals(type);
CREATE INDEX idx_efficiency_goals_active ON efficiency_goals(is_active) WHERE is_active = TRUE;
CREATE INDEX idx_efficiency_goals_deadline ON efficiency_goals(deadline) WHERE is_active = TRUE;
CREATE INDEX idx_efficiency_goals_user_active ON efficiency_goals(user_id, is_active) WHERE is_active = TRUE;

-- Efficiency metrics cache indexes
CREATE INDEX idx_efficiency_metrics_cache_user_id ON efficiency_metrics_cache(user_id);
CREATE INDEX idx_efficiency_metrics_cache_expires ON efficiency_metrics_cache(expires_at);
...
-- Create sample efficiency goals for testing
