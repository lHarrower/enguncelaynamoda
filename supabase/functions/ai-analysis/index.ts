...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!parsed.success) {
      return json(400, { ok:false, stage:"input_validation", requestId, errors: parsed.error.flatten() });
    }
    ...
    const upRes  = await fetch(cloudUrl, { method: "POST", body: fd, signal: controller.signal })
  ...
  console.log(JSON.stringify({ level: "info", requestId, stage: "analysis_done", durationMs, itemId, user: user.id }));
  return json(200, { ok:true, requestId, durationMs, stage:"analysis_done", user:user.id, itemId, imageHost:u.hostname, cloudinary:{ url: cdnUrl }, analysis });
  } catch (e) {
